    ...
    <section>
      {
        pin && (
          <div className='flex flex-col items-center bg-gray-100 p-4 min-h-screen'>
            {
              loading ? (<Loading />) : (
                <div className="bg-white rounded-lg shadow-lg max-h-auto flex flex-wrap w-full max-w-5xl mt-[2rem]">
                  <div className="w-full md:w-1/2 h-full bg-gray-200 rounded-t-lg md:rounded-l-lg md:rounded-t-none flex items-center justify-center">
                    <img
                      src={pin?.image?.url}
                      alt="Pin"
                  ...
                  <div className="w-full md:w-1/2 p-6 flex flex-col ">
                    ...
                    <ul className="overflow-y-auto hide-scrollbar h-32 md:h-64 flex flex-col mt-2">
                      {pin?.comments && pin?.comments?.length > 0 ? (
                        pin.comments.map((commentItem) => (
                          <li className="flex items-center justify-between" key={commentItem._id}>
                            ...
                            {commentItem.user === user._id && (
                              <button
                                ...
                                className="bg-red-500 text-white py-2 px-3 rounded hover:cursor-pointer"
                              >
                                <MdDelete />
                              </button>
                            )}
